...
from model_vit.transformer import VIT
from torch.utils.data.dataloader import DataLoader
from dataset_vit.mnist_color_texture_dataset import MnistDataset
...
def train_for_one_epoch(epoch_idx, model, mnist_loader, optimizer):
    ...
    :param mnist_loader: Data loader for mnist
    :param optimizer: optimizer to be used from config
    :return:
    '''
    ...
    print('Finished epoch : {}, Number_loss : {:.4f}'.format(epoch_idx+1, np.mean(losses)))
...
def train(args):
    ...
    for epoch_idx in range(num_epochs):
        mean_loss = train_for_one_epoch(epoch_idx=epoch_idx, model=model, mnist_loader=mnist_loader, optimizer=optimizer)
        scheduler.step(mean_loss)

        # Update the checkpoint if found better version
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', dest='config_path',
                        default='config_vit/default.yaml', type=str)
